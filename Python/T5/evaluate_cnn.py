...
def run_generate():
    ...
    source_lns = [x.rstrip() for x in open(args.input_path, encoding="utf8").readlines()]

    generate_summaries(source_lns, args.output_path, args.model_size, args.batch_size, args.device)

    output_lns = [x.rstrip() for x in open(args.output_path, encoding="utf8").readlines()]
    reference_lns = [x.rstrip() for x in open(args.reference_path, encoding="utf8").readlines()]
