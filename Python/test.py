...
# Load model
output_dir = "C:/Users/jbetk/Documents/data/ml/saved_models/xlnet_title_generation/local_title_clustered_progressive"
# output_dir = "C:/Users/jbetk/Documents/data/ml/saved_models/xlnet_xsum/colab_initial_go"
...
def create_inputs_for_chunk(
    ...
    _token_types = torch.cat(
        [
            torch.zeros(_outputs_so_far.shape[0], dtype=torch.long),
            torch.ones(_chunk.shape[0], dtype=torch.long),
        ]
    ).unsqueeze(dim=0)
    _inputs = {
...
def predict_words(
    ...
    for _chunk in _tok_text_chunked:
        _inputs = create_inputs_for_chunk(
            _chunk, _outputs_so_far, _mems, _tokenizer, _test_device
        )
...
def predict_forward(
    ...
    #### This is some helpful debugging
...
def test_model(
    ...
    with torch.no_grad():
        _tok_text = torch.tensor(
            _tokenizer.encode(
                _text_input + _tokenizer.eos_token, add_special_tokens=False
            ),
            dtype=torch.long,
        )
        _tok_title = torch.full(
            (_title_len,), _tokenizer.mask_token_id, dtype=torch.long
        )
        _tok_title[0] = tokenizer.bos_token_id
        _predicted_tensor = predict_forward(
            _tok_text,
            _tok_title,
            1,
            _tokenizer,
