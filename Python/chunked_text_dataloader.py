...
class ChunkedTextDataset(Dataset):
    ...
    def __init__(self, data_file: str, tokenizer: transformers.PreTrainedTokenizer,
                 max_chunk_len=192, max_gen_len=64, mask_percentage=.3, mask_all=False, pad_left=False):
    ...
    def process_element(self, text, target):
        ...
        with torch.no_grad():
            ...
            text_len_per_chunk = self.max_chunk_len - target_len - self.special_tokens_per_chunk
            ...
            padding_tensor = torch.full((padding_needed,), fill_value=self.tokenizer.pad_token_id, dtype=torch.long)
            ...
            labels = torch.full((final_text_seq_len,), fill_value=-100, dtype=torch.long)
            ...
            token_type_ids = []
            labels = []
            for c_text, c_att, c_tok, c_lab in zip(chunked_text, chunked_attention, chunked_tokens, chunked_labels):
                ...
                input_ids.append(
                    torch.cat([bos_token_tensor, c_text, sep_token_tensor, target, eos_token_tensor], dim=0))
                attention_mask.append(
                    torch.cat([one_token_float_tensor, c_att, torch.ones(target_len + 2, dtype=torch.float)], dim=0))
                token_type_ids.append(torch.cat(
                    [zero_token_tensor, c_tok, two_token_tensor, torch.ones(target_len + 1, dtype=torch.long)], dim=0))

                c_text_masked = torch.cat([bos_token_tensor, c_text, sep_token_tensor, target_masked, eos_token_tensor],
                                          dim=0)
                c_lab_full = torch.cat([n100_token_tensor, c_lab, n100_token_tensor, label_append, n100_token_tensor],
                                       dim=0)
    ...
    def perform_mask(self, tensor: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        tensor[indices_replaced] = self.tokenizer.convert_tokens_to_ids(self.tokenizer.mask_token)
    ...
    def __getitem__(self, index):
        return self.process_element(self.raw_data[index]["text"], self.raw_data[index]["target"])

    def __len__(self):
        return len(self.raw_data)

    def get_dataloader(self, batch_sz: int, num_workers=1):
        return DataLoader(self, batch_sampler=ChunkedTextBatchSampler(self, batch_sz), shuffle=False, num_workers=num_workers)
...
class ChunkedTextBatchSampler(Sampler):
    def __init__(self, chunked_text_set: ChunkedTextDataset, batch_sz: int, drop_last=True):
...
if __name__ == "__main__":
    ...
    test_file = "C:\\Users\\jbetk\\Documents\\data\\ml\\title_prediction\\outputs\\val.pt"
